...
export const api = {
  ...
  async issueCertificate(input: { docId?: string; title?: string; reason?: string; ownerId?: string; studentId?: string; studentEmail?: string; studentName?: string; studentRef?: string; kind?: string; file: File }, onProgress?: (pct: number) => void): Promise<any> {
    const token = sessionStorage.getItem('token');
    const form = new FormData();
