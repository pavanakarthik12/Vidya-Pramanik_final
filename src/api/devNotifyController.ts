...
export async function registerDevNotifyRoutes(app: FastifyInstance) {
  ...
  const subject = typeof body.subject === 'string' ? body.subject : '[DEV] Vidya Pramanik Email Test';
  const text = typeof body.text === 'string' ? body.text : 'Hello from Vidya Pramanik dev email test.';
